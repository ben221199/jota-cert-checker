...
terminal_mode(){
	printf "\n| %-50s | %-50s | %-30s | %-10s | %-5s %s\n" "SITE" "SNI" "EXPIRATION DAY" "DAYS LEFT" "STATUS"

	while read site;do
		sitename=$(echo $site | cut -d ":" -f1)
		certificate=$(echo | openssl s_client -servername ${sitename} -connect ${site} 2>/dev/null)
		sni=$(echo "${certificate}" | openssl x509 -noout -subject 2>/dev/null)
		# | cut -d "=" -f2)
		certificate_last_day=$(echo "${certificate}" | openssl x509 -noout -enddate 2>/dev/null | cut -d "=" -f2)
		end_date=$(date +%s -d "$certificate_last_day")
		days_left=$(((end_date - current_date) / 86400))

		if [ "$days_left" -gt "$warning_days" ];then
			printf "${ok_color}| %-50s | %-50s | %-30s | %-10s | %-5s %s\n${end_of_color}" \
			"$site" "$sni" "$certificate_last_day" "$days_left" "Ok"

		elif [ "$days_left" -le "$warning_days" ] && [ "$days_left" -gt "$alert_days" ];then
			printf "${warning_color}| %-50s | %-50s | %-30s | %-10s | %-5s %s\n${end_of_color}" \
			"$site" "$sni" "$certificate_last_day" "$days_left" "Warning"

		elif [ "$days_left" -le "$alert_days" ] && [ "$days_left" -gt 0 ];then
			printf "${alert_color}| %-50s | %-50s | %-30s | %-10s | %-5s %s\n${end_of_color}" \
			"$site" "$sni" "$certificate_last_day" "$days_left" "Alert"

		elif [ "$days_left" -le 0 ];then
			printf "${expired_color}| %-50s | %-50s | %-30s | %-10s | %-5s %s\n${end_of_color}" \
			"$site" "$sni" "$certificate_last_day" "$days_left" "Expired"
		fi
